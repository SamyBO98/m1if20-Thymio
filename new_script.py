...
def Braitenberg():
    ...
    horizontal = network.GetVariable("thymio-II", "prox.horizontal")#,reply_handler=get_variables_reply,error_handler=get_variables_error)
    left = horizontal[0]
    center = horizontal[2]
    right = horizontal[4]
    ...
    if center != 0:
        ...
        if right != 0:
            network.SetVariable("thymio-II", "motor.left.target", [0])
            network.SetVariable("thymio-II", "motor.right.target", [motor])
        elif left != 0:
            network.SetVariable("thymio-II", "motor.left.target", [motor])
            network.SetVariable("thymio-II", "motor.right.target", [0])
        else:
            network.SetVariable("thymio-II", "motor.left.target", [motor])
            network.SetVariable("thymio-II", "motor.right.target", [motor])
